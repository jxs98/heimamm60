  ...
  <div id="enterprise">
    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-form-item label="创建者" prop="username">
          <el-input
            v-model="enterpriseList.username"
            style="width: 149px"
          ></el-input>
        </el-form-item>
    ...
    <el-card class="enterpriselist">
      ...
      >
        ...
        <el-table-column prop="remark" label="企业备注"> </el-table-column>
        <el-table-column label="状态">
          ...
          <template slot-scope="scope">
            ...
            <el-switch
              ...
              :active-value="1"
              :inactive-value="0"
              @change="enterpriseStatus(scope.row)"
            >
            </el-switch>
          </template>
        </el-table-column>
      ...
      <el-pagination
        ...
        :page-size="enterpriseList.limit"
...
export default {
  ...
  methods: {
    ...
    async getAllEnterprise () {
      const { data: res } = await this.$http.get('/enterprise/list', { params: this.enterpriseList })
      if (res.code === 200) {
    ...
    handleCurrentChange (newpage) {
      //  页码重新赋值
      this.enterpriseList.page = newpage
      // 重新渲染
    ...
    handleSizeChange (newpage) {
      //  页尺寸重新赋值
      this.enterpriseList.limit = newpage
      // 重新渲染
      this.getAllEnterprise()
    },
    //  用户状态切换
    async enterpriseStatus (status) {
      const { data: res } = await this.$http.post('/enterprise/status', { id: status.id })
      if (res.code !== 200) {
        status.status = status.status === 0 ? 1 : 0
        return this.$message.error('企业状态更新失败!')
      }
      this.$message.success('企业状态更新成功!')
    },
    //  搜索按钮点击事件
    search () {
      this.enterpriseList.page = 1
      this.getAllEnterprise()
    ...
    del (row) {
      this.$confirm(`确认删除${row.name}?`, '提示', {
        ...
        const { data: res } = await this.$http.post('/enterprise/remove', { id: row.id })
        if (res.code === 200) {
          this.getAllEnterprise()
          this.$message({
            type: 'success',
